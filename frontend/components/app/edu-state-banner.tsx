...
function MicIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M19 12C19 15.87 15.87 19 12 19C8.13 19 5 15.87 5 12H3C3 16.97 6.47 21.16 11 21.9V24H13V21.9C17.53 21.16 21 16.97 21 12H19Z" fill="currentColor"/>
    </svg>
  );
}
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        ...
        >
          ...
          {config.showWave && (
            <span className="flex items-end gap-0.5 ml-1 h-4">
              <span className="animate-wave-1 inline-block w-1 rounded-sm bg-current" style={{ height: '60%' }} />
              <span className="animate-wave-2 inline-block w-1 rounded-sm bg-current" style={{ height: '100%' }} />
              <span className="animate-wave-3 inline-block w-1 rounded-sm bg-current" style={{ height: '80%' }} />
              <span className="animate-wave-4 inline-block w-1 rounded-sm bg-current" style={{ height: '40%' }} />
              <span className="animate-wave-5 inline-block w-1 rounded-sm bg-current" style={{ height: '70%' }} />
            </span>
          )}
        </div>
        ...
        {config.sublabel && (
          <p className="max-w-xs text-center text-xs text-red-400/80 leading-relaxed px-2">
            {config.sublabel}
          </p>
        )}
      </motion.div>
